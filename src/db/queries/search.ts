 ...
 */
export async function searchWorkspace(workspaceId: string, q: string, limits: { rfps: number; questions: number } = SEARCH_LIMITS, executor: Executor = db): Promise<SearchResults> {
  ...
  const [rfpRows, questionRows] = await Promise.all([
    ...
    executor
      ...
      .where(and(eq(rfpQuestions.workspaceId, workspaceId), or(ilike(rfpQuestions.refNo, pattern), ilike(rfpQuestions.questionText, pattern))))
